...
ROUTE_TMPL = PromptTemplate(
    ...
    "  'баярлалаа'               -> chat\n\n"
    "standalone_query: rewrite Query into ONE self-contained question by resolving\n"
    "everything it borrows from the recent conversation — pronouns, a dropped\n"
    "subject/object, 'тэр/энэ/түүнийг', or elliptical follow-ups ('...аас авч болох "
    "уу?', 'хэдэн төгрөг вэ?'). Keep the user's wording and language (Mongolian); do\n"
    "NOT answer it, expand it, or add facts not implied by the history. If Query is\n"
    "already self-contained, copy it verbatim.\n"
    "  history mentions 'ААН-ын тоон гарын үсэг', Query 'и-монголоос захиалж болох уу'\n"
    "    -> 'и-монголоос ААН-ын тоон гарын үсэг захиалж болох уу'\n\n"
